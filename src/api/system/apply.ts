...
// 修改应用状态
export const changeApplyStatus_api = (id: string, data: any) =>
  request.put(`/application/${id}`, data);
// 删除应用
...
// 获取组织列表
export const getDepartmentList_api = (params: any) =>
  request.get(`/organization/_all/tree`, params);
// 获取应用详情
...
export const getOwnerStandalone_api = (appId: string, data: object) =>
  request.post(`/application/${appId}/_/api/menu/owner`, data);

// 获取对应系统菜单树
export const getOwnerTree_api = (owner: string) => request.post(`/menu/owner/tree/${owner}`, {});
export const getOwnerTreeStandalone_api = (appId: string, owner: string) =>
  request.post(`/application/${appId}/_/api/menu/owner/tree/${owner}`, {});
// 保存集成菜单
export const saveOwnerMenu_api = (owner: string, appId: string, data: object) =>
  request.patch(`/menu/owner/${owner}/${appId}/_all`, data);
