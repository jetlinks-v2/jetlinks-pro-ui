...
// 获取角色对应的权限树
export const getPermissionTree_api = (id: string): Promise<any> =>
  request.get(`/menu/role/${id}/_grant/tree`);
// 更新角色对应的权限树
export const updatePermissionTree_api = (id: string, data: any): Promise<any> =>
...
// 将用户与角色绑定
export const bindUser_api = (roleId: string, data: string[]): Promise<any> =>
  request.post(`/role/${roleId}/users/_bind`, data);
// 将用户与角色解绑
export const unbindUser_api = (roleId: string, data: string[]): Promise<any> =>
  request.post(`/role/${roleId}/users/_unbind`, data);
//查询分组
