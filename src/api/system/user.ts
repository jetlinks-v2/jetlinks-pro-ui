...
// 校验字段合法性
export const validateField_api = (type: 'username' | 'password', name: string) =>
  request.post(`/user/${type}/_validate`, name, {
    headers: {
...
export const queryUserListNoPaging = () =>
  request.post(`/user/_query/no-paging`, {
    paging: false,
...
// 更新密码
export const updatePassword_api = (data: { id: string; password: string }) =>
  request.post(`/user/${data.id}/password/_reset`, data.password, {
    headers: {
      'Content-Type': 'text/plain',
    },
  });
...
// 查询角色不分页
export const queryRole_api = (data: any): Promise<any> =>
  request.post(`/role/_query/no-paging`, data);
